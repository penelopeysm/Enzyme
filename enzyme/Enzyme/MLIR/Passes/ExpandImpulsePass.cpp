...
struct ExpandImpulsePass
  ...
  struct LowerMCMCPattern : public mlir::OpRewritePattern<impulse::InferOp> {
    ...
    LogicalResult matchAndRewrite(impulse::InferOp mcmcOp,
      ...
      auto expectedDiagnosticsType =
          cast<RankedTensorType>(mcmcOp.getResult(1).getType());
      if (finalDiagnosticsBuffer.getType() != expectedDiagnosticsType) {
        finalDiagnosticsBuffer = impulse::ReshapeOp::create(
            rewriter, loc, expectedDiagnosticsType, finalDiagnosticsBuffer);
      }
